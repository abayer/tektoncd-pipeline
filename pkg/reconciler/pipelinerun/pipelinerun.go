...
// filterTaskRunsForPipelineRun takes a slice of TaskRuns with the given PipelineRun marked as owner via the "tekton.dev/pipelineRun"
// label, and returns a slice of TaskRuns directly specified in the PipelineRun and a map of condition TaskRuns keyed by
// the PipelineTask name they relate to.
...
func updatePipelineRunStatusFromChildRefs(logger *zap.SugaredLogger, pr *v1beta1.PipelineRun, trs []*v1beta1.TaskRun, runs []*v1alpha1.Run) {
	...
	for pipelineTaskName, actualConditionTaskRuns := range conditionTaskRuns {
		...
		for k, v := range getNewConditionChecksForTaskRun(logger, childRefByPipelineTask[pipelineTaskName].GetConditionChecksAsMap(), actualConditionTaskRuns) {

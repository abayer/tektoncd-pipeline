...
func createRequest(repoURL, pathInRepo, branch, specificCommit string, useNthCommit int, commitsByBranch map[string][]string) *v1alpha1.ResolutionRequest {
	rr := &v1alpha1.ResolutionRequest{
		...
		Spec: v1alpha1.ResolutionRequestSpec{
			Params: []v1alpha1.ResolutionParam{
				{
					Name:  URLParam,
					Value: repoURL,
				}, {
	...
	if branch != "" {
		rr.Spec.Params = append(rr.Spec.Params, v1alpha1.ResolutionParam{
			Name:  BranchParam,
			Value: branch,
		})
	}
	...
	if useNthCommit > 0 {
		rr.Spec.Params = append(rr.Spec.Params, v1alpha1.ResolutionParam{
			Name:  CommitParam,
			Value: commitsByBranch[plumbing.Master.Short()][useNthCommit],
		})
	} else if specificCommit != "" {
		rr.Spec.Params = append(rr.Spec.Params, v1alpha1.ResolutionParam{
			Name:  CommitParam,
			Value: hex.EncodeToString([]byte(specificCommit)),
		})
	}

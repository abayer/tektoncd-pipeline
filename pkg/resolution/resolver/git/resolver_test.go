...
func TestValidateParams(t *testing.T) {
	...
	if err := resolver.ValidateParams(context.Background(), paramsWithRevision); err != nil {
...
func createRequest(repoURL, pathInRepo, revision, specificCommit string, useNthCommit int, commitsByBranch map[string][]string) *v1alpha1.ResolutionRequest {
	...
	switch {
	case useNthCommit > 0:
		rr.Spec.Params[RevisionParam] = commitsByBranch[plumbing.Master.Short()][useNthCommit]
	case specificCommit != "":
		rr.Spec.Params[RevisionParam] = hex.EncodeToString([]byte(specificCommit))
	case revision != "":
		rr.Spec.Params[RevisionParam] = revision
	}
